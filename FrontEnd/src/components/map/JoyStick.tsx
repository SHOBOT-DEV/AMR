...
const Joystick: React.FC<JoystickProps> = ({
  ...
  // ✅ STABLE onMove reference (CRITICAL FIX)
  const onMoveRef = useRef<typeof onMove>();
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fullyStopKeyboard = () => {
      Object.keys(keyStateRef.current).forEach(
        (k) => ((keyStateRef.current as any)[k] = false)
      );

      const stop: JoystickData = {
        x: 0,
        y: 0,
        force: 0,
        angle: 0,
    ...
    const computeKeyboardMovement = () => {
      ...
      const vx =
        (lettersActive ? (s.d ? 1 : 0) : s.ArrowRight ? 1 : 0) -
        (lettersActive ? (s.a ? 1 : 0) : s.ArrowLeft ? 1 : 0);

      const vy =
        (lettersActive ? (s.s ? 1 : 0) : s.ArrowDown ? 1 : 0) -
        (lettersActive ? (s.w ? 1 : 0) : s.ArrowUp ? 1 : 0);

      const payload: JoystickData = {
        x: vx,
        y: -vy,
        force: vx || vy ? 1 : 0,
        angle: vx || vy ? (Math.atan2(-vy, vx) * 180) / Math.PI : 0,
        type: "keyboard",
      };

      const last = lastSentRef.current;
      if (!last || last.x !== payload.x || last.y !== payload.y) {
      ...
      if (canvas && joystickRef.current?.SetPosition) {
        ...
        if (payload.force) {
          joystickRef.current.SetPosition(
            rect.left + rect.width / 2 + vx * maxR,
            rect.top + rect.height / 2 + vy * maxR
          );
    ...
    const onKey = (down: boolean) => (e: KeyboardEvent) => {
      ...
      if (isUILocked()) return fullyStopKeyboard();
      if (anyEditableFocused()) return;

      const k = e.key.toLowerCase();
      if (k in keyStateRef.current) (keyStateRef.current as any)[k] = down;
      if (e.key in keyStateRef.current)
        (keyStateRef.current as any)[e.key] = down;
